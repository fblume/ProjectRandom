...
var owner = false;
  ...
  $(document).ready(function(){
    window.ethereum.enable().then(function(accounts){
      contractInstance = new web3.eth.Contract(abi, "0x293d2ee5d063660DAA6F8F7b0B1da2592A672F57", {from: accounts[0]});
        ...
        function submitAndPlay(){
          ...
          contractInstance.methods.getNetContractBalance().call().then(function(netBalance){
            if(1*netBalance > 1*web3.utils.toWei(potentialGainLoss.toString(), "ether")){
              ...
              contractInstance.methods.receiveBet(moreOnZero).send(config);
              checkEvent();
            }else{
          ...
          function checkEvent(){
            contractInstance.once("generateRandomNumber", function(error, eventResult){
              if(!error){
                contractInstance.methods.getQueryId().call({from: customerAddress}).then(function(queryIdResult){
